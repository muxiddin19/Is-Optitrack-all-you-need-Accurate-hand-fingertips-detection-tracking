...
CHESSBOARD_SIZE = (7, 4)  # Number of inner corners per chessboard row and column
SQUARE_SIZE = 3.3         # Size of a square in centimeters
CALIBRATION_IMAGES_PATH = 'calibration_images/*.jpg'  # Path to calibration images
OUTPUT_DIRECTORY = 'output1'  # Directory to save calibration results
...
def calibrate_camera():
    ...
    if not objpoints or not imgpoints:
        print("Error: No valid chessboard patterns detected! Ensure correct board size and visibility.")
        return None, None, None, None, None
    ...
    with open(os.path.join(OUTPUT_DIRECTORY, 'calibration_data1.pkl'), 'wb') as f:
        pickle.dump(calibration_data, f)
    
    # Save camera matrix and distortion coefficients as text files
    np.savetxt(os.path.join(OUTPUT_DIRECTORY, 'camera_matrix1.txt'), mtx)
    np.savetxt(os.path.join(OUTPUT_DIRECTORY, 'distortion_coefficients1.txt'), dist)
...
def main():
    ...
    if ret:
        print(f"Camera successfully calibrated! Reprojection error: {ret}")
    else:
        print("Calibration failed. Check image quality and chessboard visibility.")
