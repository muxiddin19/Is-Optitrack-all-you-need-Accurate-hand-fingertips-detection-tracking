...
class SimpleD415ToNeWCRFs:
    def __init__(self):
        ...
        self.color_path = r"E:\vscode\calibration\data\user05\2\d415\rgb"
        self.depth_path = r"E:\vscode\calibration\data\user05\2\d415\depth"
        
        # Output path for NeWCRFs
        self.output_path = r"E:\vscode\newcrfs_training_data/415_5_2"
    ...
    def check_data(self):
        ...
        # Find all PNG files (your actual file pattern)
        color_files = glob.glob(os.path.join(self.color_path, "*.png"))
        depth_files = glob.glob(os.path.join(self.depth_path, "*.png"))
        ...
        if len(color_files) == 0 or len(depth_files) == 0:
            ...
            print("\n🔍 Let's check what's actually in these folders:")
            
            print(f"\n📁 Contents of {self.color_path}:")
            try:
                items = os.listdir(self.color_path)
            ...
            print(f"\n📁 Contents of {self.depth_path}:")
            try:
                items = os.listdir(self.depth_path)
                for item in items[:10]:  # Show first 10 items
                    print(f"   {item}")
                if len(items) > 10:
                    print(f"   ... and {len(items)-10} more items")
    ...
    def find_matching_pairs(self):
        """Find matching color-depth pairs based on your naming pattern"""
        ...
        # Extract frame numbers from your naming pattern: "000000_rgb_20250526_095608.png"
        for color_file in color_files:
            filename = Path(color_file).name
            # Extract frame number (first part before underscore)
            frame_num = filename.split('_')[0]  # "000000"
        ...
        if matched_pairs:
            ...
            for i, (color_file, depth_file) in enumerate(matched_pairs[:3]):
                print(f"   Pair {i+1}: {Path(color_file).name} <-> {Path(depth_file).name}")
    ...
    def generate_newcrfs_command(self):
        ...
        # Convert Windows path to Linux path for the command
        linux_output_path = self.output_path.replace("E:\\vscode\\", "/home/spacetop/")
...
def main():
    ...
    if success:
        ...
        print(f"   scp -r {converter.output_path} username@server:/home/spacetop/")
