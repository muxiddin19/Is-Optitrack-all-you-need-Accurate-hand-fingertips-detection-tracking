...
class SimpleMultiCameraCapture:
    ...
    def __init__(self, output_dir="simple_capture"):
        ...
        # Create separate directories for each camera type
        ...
        print(f"  🎯 D415: {self.d415_dir}")
        print(f"  🎯 D405: {self.d405_dir}")
        print(f"  🎯 Platform: {self.platform_dir}")
        print(f"  🎯 Synchronized: {self.sync_dir}")
    ...
    def save_frame_set(self, frames):
        """Save current frame set to organized directories"""
        ...
        # Save synchronized frame set (all cameras in one image)
        if len(frames) > 1:
        ...
        self.frame_count += 1
        print(f"📸 Set {self.frame_count}: {saved_count} files saved ({', '.join(saved_details)})")
